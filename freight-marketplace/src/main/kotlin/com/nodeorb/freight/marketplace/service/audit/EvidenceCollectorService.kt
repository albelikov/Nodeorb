package com.nodeorb.freight.marketplace.service.audit
...
import com.nodeorb.freight.marketplace.service.ScoringService
...
class EvidenceCollectorService(
    ...
    private fun getBidByContract(contract: ContractEntity): BidEntity {
        return bidRepository.findById(contract.bidId)
            ?: throw IllegalArgumentException("Bid not found for contract: ${contract.id}")
    }
    ...
    private fun getScmSnapshotByBid(bid: BidEntity): ScmSnapshotEntity {
        return scmSnapshotRepository.findByBidId(bid.id!!)
    ...
    fun getEvidenceData(contractId: UUID): String {
        val contract = bidRepository.findById(contractId)
            ?: throw IllegalArgumentException("Contract not found: $contractId")
