...
import com.nodeorb.freight.marketplace.service.AnalyticsService
...
class AnalyticsController(
    ...
    fun getOrderStats(
        ...
        try {
            ...
            val stats = mapOf(
                "totalBids" to bids.size,
                "avgMatchingScore" to bids.mapNotNull { it.matchingScore }.average().takeIf { it.isFinite() } ?: 0.0,
                "avgAmount" to bids.map { it.amount.toDouble() }.average().takeIf { it.isFinite() } ?: 0.0,
                "minAmount" to bids.minByOrNull { it.amount }?.amount,
                "maxAmount" to bids.maxByOrNull { it.amount }?.amount,
                "dangerousGoodsQualified" to bids.count { it.passesDangerousGoodsRules },
                "avgCarrierRating" to bids.map { it.carrierRating }.average().takeIf { it.isFinite() } ?: 0.0,
                "avgOnTimeDeliveryRate" to bids.map { it.onTimeDeliveryRate }.average().takeIf { it.isFinite() } ?: 0.0
            )
